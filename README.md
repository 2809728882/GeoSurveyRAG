...
- **三种回答模式**：支持“纯本地知识库匹配”“知识库 + AI”“知识库 + AI + 联网采集”，方便演示不同业务场景下的 RAG 链路。
- **知识库管理界面**：支持网页采集、文件上传、手动录入、默认来源管理、入库文档列表和任务状态追踪。
- **自适应网页爬虫**：支持 URL 自动规范化、HTML 正文抽取、PDF/JSON/纯文本解析、编码识别、标题识别、最终跳转 URL 记录和失败状态反馈。
